...
def probe_clip(m2ts_path: str) -> Clip:
    ...
    cmd = [
        "ffprobe",
        "-v", "error",
        "-select_streams", "v:0,a,s",
        "-show_entries", "format=duration:stream=index,codec_type,codec_name,width,height,r_frame_rate,bit_rate,channel_layout",
        "-of", "json",
        m2ts_path,
    ]
...
def to_json(inventory: Inventory) -> str:
    ...
    data = {
        "clips": {
            clip_id: asdict(clip)
            for clip_id, clip in inventory.clips.items()
        },
        "playlists": {
            pl_id: asdict(pl)
            for pl_id, pl in inventory.playlists.items()
        },
    }
...
def from_json(json_str: str) -> Inventory:
    ...
    for clip_id, clip_data in data.get("clips", {}).items():
        ...
        audio = [
            AudioStream(**a) for a in clip_data.get("audio", [])
        ]
        
        subtitles = [
            SubtitleStream(**s) for s in clip_data.get("subtitles", [])
        ]

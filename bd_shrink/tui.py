...
def select_source(console: Console, source_root: str = "") -> Optional[str]:
    ...
    confirm = questionary.confirm(
        ...
        default=True,
        auto_enter=False,
        style=questionary.Style([("question", f"fg:{ColorTheme.BLUE} bold")]),
    ).ask()
    ...
    while True:
        start_dir = (
            source_root if (source_root and os.path.isdir(source_root)) else str(Path.home())
        )
...
def select_output(console: Console, source: str, default_dir: str = "") -> Optional[str]:
    ...
    choice = questionary.select(
        ...
        style=questionary.Style([("pointer", f"fg:{ColorTheme.BLUE} bold")]),
    ).ask()
...
def select_mode(console: Console, current_movie_only: bool = False) -> Optional[bool]:
    ...
    default = (
        ...
        if current_movie_only
        else "Full disc (keep menus, extras)"
    )

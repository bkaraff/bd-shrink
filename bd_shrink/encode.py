...
def extract_audio(
    ...
    first_audio = os.path.join(
        encode_dir,
        f"{clip.clip_id}_audio_0{track_exts[0]}"
    ) if track_exts else None
    ...
    if result.succeeded and first_audio and os.path.isfile(first_audio) and os.path.getsize(first_audio) > 0:
        # Verify all tracks exist
        audio_tracks = actual_audio_idx
        for ai in range(actual_audio_idx):
            ext = track_exts[ai]
...
def extract_subtitles(
    ...
    for si, codec in enumerate(sub_codecs):
        ...
        ext = audio_ext(codec)  # Reusing audio_ext for subtitle extension lookup
        fmt = tsmuxer_type(codec)  # Format specifier for ffmpeg -f option
    ...
    first_sub = os.path.join(encode_dir, f"{clip.clip_id}_sub_0.sup") if actual_sub_idx > 0 else None
    if skip_if_exists(out_file=first_sub):
        return actual_sub_idx
    
    result = run_managed(sb_args, logger=logger)
    
    if result.succeeded and first_sub and os.path.isfile(first_sub) and os.path.getsize(first_sub) > 0:
        return actual_sub_idx
...
def encode_video_single_pass(
    ...
    if is_main:
        ...
        crf_or_bitrate = ["-b:v", config.main_bitrate, "-maxrate", config.main_maxrate, "-bufsize", config.main_bufsize]
    else:
        # Extras: use CRF
        preset = "medium"
...
def encode_video_two_pass(
    ...
    for attempt in range(3):
        cmd = [
            ...
            "-preset", preset,
            "-b:v", config.main_bitrate,
            "-maxrate", config.main_maxrate,
            "-bufsize", config.main_bufsize,
        ]
        if not is_hevc:
            cmd += ["-x264opts", "bluray-compat=1"]
        cmd += [
...
def encode_clip(
    ...
    if clip.video is None or clip.video.height == 0:
        if logger:
            logger.warning(f"[{clip_idx}/{total_clips}] {clip_type.upper()}: {clip.clip_id}.m2ts — skipping (no video)")
        return EncodeStats(
            clip_name=clip.clip_id,
            clip_type=clip_type,
            audio_tracks=0,
